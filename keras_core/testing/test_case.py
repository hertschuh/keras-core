        ...
        def run_output_asserts(layer, output, eager=False):
            ...
            if expected_output_sparse:
                ...
                for x in tree.flatten(output):
                    if isinstance(x, keras_tensor.KerasTensor):
...
def create_keras_tensors(input_shape, dtype, sparse):
    if isinstance(input_shape, tuple):
        return keras_tensor.KerasTensor(input_shape, dtype=dtype, sparse=sparse)
    if isinstance(input_shape, list):
        return [
            keras_tensor.KerasTensor(s, dtype=dtype, sparse=sparse)
...
def create_eager_tensors(input_shape, dtype, sparse):
    ...
    if sparse:
        ...
        create_values_fn = create_fn

        def create_fn(shape, dtype):
            values_count = min(dim for dim in shape if dim > 1)
            indices = []
            for i in range(values_count):
                indices.append(
                    [
                        i if dim == values_count else np.random.randint(0, dim)
                        for dim in shape
                    ]
                )
            ...
            return tf.sparse.reorder(
                tf.SparseTensor(
                    indices=indices,
                    values=create_values_fn((values_count,), dtype=dtype),
                    dense_shape=shape,
                )
            )
